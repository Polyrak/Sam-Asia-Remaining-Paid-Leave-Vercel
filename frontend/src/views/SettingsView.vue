          ...
          <v-expansion-panels multiple variant="accordion">
            <v-expansion-panel title="Projects (find OP_LEAVE_PROJECT_ID)">
              <v-expansion-panel-text>
                <div v-for="p in discovery.projects" :key="p.id">#{{ p.id }} — {{ p.name }} ({{ p.identifier }})</div>
              </v-expansion-panel-text>
            </v-expansion-panel>
      ...
      <v-card-text>
        ...
        <v-list>
          <v-list-item v-for="(override, userId) in entitlements.overrides" :key="userId">
            ...
            <template #append>
              <v-btn icon="mdi-close" size="small" variant="text" @click="clearOverride(userId)" />
            </template>
          </v-list-item>
        </v-list>
        ...
        <div class="d-flex align-center ga-4 mt-4 flex-wrap">
          ...
          <v-text-field
            ...
            hint="Only needed to override what's shown — a formula typed in Adjustment is already preserved automatically"
            persistent-hint
            density="compact"
            variant="outlined"
            style="max-width: 200px"
          />
          <v-btn color="primary" :loading="savingOverride" @click="saveOverride">Save override</v-btn>
        </div>
      </v-card-text>
    </v-card>
    ...
    <v-card class="mt-6" variant="outlined">
      ...
      <v-card-text>
        ...
        <v-textarea
          ...
          hide-details
        />
        <v-btn color="primary" class="mt-4" :loading="orderSaving" @click="saveOrder">Save order</v-btn>
      </v-card-text>
    </v-card>
  </v-container>
</template>
